    ...
    names(data.df) %>%
      purrr::walk(function(df) {
        l <<- l %>% leaflet::addAwesomeMarkers(
          ...
          label = ~ glue::glue(
            "{lubridate::with_tz(observation_start_time, tz)} {humanize(status_health)}",
            " {humanize(details_maturity)} {humanize(details_sex)} {humanize(details_species)}"
          ),
          popup = ~ glue::glue(
            ...
            "Cause of death: {humanize(death_cause_of_death)} ({humanize(death_cause_of_death_confidence)})",

            "<h5>Animal</h5>",
            "Activity: {status_activity}; behaviour: {status_behaviour} <br/>",

            '<img height="150px;" alt="Photo carapace top" src="{ifelse(!is.na({photos_turtle_photo_carapace_top}), photos_turtle_photo_carapace_top, "")}"></img><br/>',
            '<img height="150px;" alt="Photo head top" src="{ifelse(!is.na({photos_turtle_photo_head_top}), photos_turtle_photo_head_top, "")}"></img><br/>',
            '<img height="150px;" alt="Photo head side" src="{ifelse(!is.na({photos_turtle_photo_head_side}), photos_turtle_photo_head_side, "")}"></img><br/>',
            '<img height="150px;" alt="Photo head front" src="{ifelse(!is.na({photos_turtle_photo_head_front}), photos_turtle_photo_head_front, "")}"></img><br/>',

            "<h5>Habitat</h5>",
            '<img height="150px;" alt="Photo Habitat 1" src="{ifelse(!is.na({incident_photo_habitat}), incident_photo_habitat, "")}"></img><br/>',
            '<img height="150px;" alt="Photo Habitat 2" src="{ifelse(!is.na({habitat_photos_photo_habitat_2}), habitat_photos_photo_habitat_2, "")}"></img><br/>',
            '<img height="150px;" alt="Photo Habitat 3" src="{ifelse(!is.na({habitat_photos_photo_habitat_3}), habitat_photos_photo_habitat_3, "") }"></img><br/>',
            '<img height="150px;" alt="Photo Habitat 4" src="{ifelse(!is.na({habitat_photos_photo_habitat_4}), habitat_photos_photo_habitat_4, "") }"></img><br/>'
          ),
          group = df,
          clusterOptions = co
        )
